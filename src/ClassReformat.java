...
public class ClassReformat {
    ...
    private static boolean absorption;
...
//    }
    ...
    private static void calculateTotalRent(){
        DataFormat cf = wb.createDataFormat();
        CellStyle currencyCellStyle = wb.createCellStyle();
        currencyCellStyle.setDataFormat(cf.getFormat("$#,##0.00"));
        int subRentIndex = getHeaderIndex(wb,"Weighted Sublease Average");
        int dirRentIndex = getHeaderIndex(wb,"Weighted Direct Average");
        int SASIndex = getHeaderIndex(wb,"Sublet Available Space");
        int outputIndex = getHeaderIndex(wb, "Weighted Overall Average");
        int DASindex = getHeaderIndex(wb, "Direct Available Space");
        for (int key = 0; key < wb.getNumberOfSheets(); key++) {
            Sheet market = wb.getSheetAt(key);
            for (int i = 1; i <= market.getLastRowNum(); i++) {
                Row r = market.getRow(i);
                Cell DASCell = r.getCell(DASindex, Row.RETURN_BLANK_AS_NULL);
                Cell SASCell = r.getCell(SASIndex, Row.CREATE_NULL_AS_BLANK);
                Cell subRentCell = r.getCell(subRentIndex, Row.RETURN_BLANK_AS_NULL);
                Cell dirRentCell = r.getCell(dirRentIndex, Row.RETURN_BLANK_AS_NULL);
                if ((DASCell != null && SASCell != null) && (subRentCell != null && dirRentCell !=null)) {
                    double DAS = DASCell.getNumericCellValue();
                    double SAS = SASCell.getNumericCellValue();
                    double subRent = subRentCell.getNumericCellValue();
                    double dirRent = dirRentCell.getNumericCellValue();
                    if (!(DAS == 0 && SAS == 0)) {
                        double numerator = ((DAS*dirRent) + (SAS*subRent));
                        double denominator = DAS + SAS;
                        double total = numerator/denominator;
                        Cell c = r.createCell(outputIndex);
                        c.setCellValue(total);
                        c.setCellStyle(currencyCellStyle);
                    }
                    else {
    ...
    private static void calculateInventory(){
        int oldINVIndex = getHeaderIndex(lastQuarterWB,"Inventory");

        int newInvIndex = getHeaderIndex(wb,"Inventory");
        for (int key = 0; key < lastQuarterWB.getNumberOfSheets(); key++) {
            Sheet market = wb.getSheetAt(key);
            Sheet oldMarket = lastQuarterWB.getSheetAt(key);
            for (int i = 1; i <= oldMarket.getLastRowNum(); i++) {
                Row r = market.getRow(i);
                Row oldRow = oldMarket.getRow(i);
                if (oldRow == null) break;
                Cell OldInvCell = oldRow.getCell(oldINVIndex, Row.RETURN_BLANK_AS_NULL);
                if (OldInvCell != null ) {
                    double oldInv = OldInvCell.getNumericCellValue();
                    if (!(oldInv == 0)) {
                        Cell c = r.createCell(newInvIndex);
                        c.setCellValue(oldInv);
                        c.setCellStyle(bigNum);
                    }
                    else {
                        Cell c = r.createCell(newInvIndex);
    ...
    private static void calculateNetAbsorption() {
        ...
        for (int key = 0; key<wb.getNumberOfSheets();key++) {
            ...
            for (int i = 1; i <= market.getLastRowNum(); i++) {
                Row r = market.getRow(i);
                Row oldR = oldMarket.getRow(i);
                Cell currentOS = r.getCell(currentOSIndex, Row.RETURN_BLANK_AS_NULL);
                Cell lastOS = oldR.getCell(lastQuarterOSIndex, Row.CREATE_NULL_AS_BLANK);
    ...
    private static void populate() {
        ...
        if (absorption) calculateNetAbsorption();
    ...
    public static void main(String [] args) throws IOException, InvalidFormatException {
        ...
        int replyForInventory = JOptionPane.showConfirmDialog(null, "Would you like to use last quarters inventory?"
                , "RBA", JOptionPane.YES_NO_OPTION);
        String lastQuarterPath = null;
        absorption = true;
        if (reply == JOptionPane.YES_OPTION || replyForInventory == JOptionPane.YES_OPTION) {

            if(Chooser.showDialog(null, "Last Quarter")== JFileChooser.APPROVE_OPTION) {
        ...
        int returnVal = Chooser.showDialog(null, "CoStar Import");
        if(returnVal == JFileChooser.APPROVE_OPTION) {
            inputPath = Chooser.getSelectedFile().getPath();
        }
        String outputFile = JOptionPane.showInputDialog("Enter desired spreadsheet output name: ");
        wb = new XSSFWorkbook();
        inputWB = WorkbookFactory.create(new File(inputPath));
        ...
        FileOutputStream fileOut = new FileOutputStream(outputPath+"/"+outputFile+".xlsx");
        wb.write(fileOut);
        fileOut.close();
